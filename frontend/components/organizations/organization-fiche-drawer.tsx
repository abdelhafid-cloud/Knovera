...
export function OrganizationFicheDrawer({ open, organizationId, onClose, onChanged }: Props) {
  ...
  const saveQuotas = async (e: FormEvent) => {
    ...
    try {
      const r = await api.patch<Organization>(`/api/organizations/${organizationId}`, {
        quotas: {
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      await api.post(`/api/organizations/${organizationId}/${action}`);
  ...
  const deleteOrg = async () => {
    if (!org || !organizationId || !confirm(`Supprimer « ${org.name} » ?`)) return;
    setBusy(true);
    try {
  ...
  const sendInvite = async (e: FormEvent) => {
    ...
    try {
      const r = await api.post<{ token?: string }>(`/api/organizations/${organizationId}/invitations`, {
        email: inviteEmail,
  ...
  return (
    <>
      <AppDrawer
        open={open}
        onClose={onClose}
        labelledBy="org-fiche-title"
        title={org?.name || "Fiche organisation"}
        className="max-w-2xl"
      >
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <section className="space-y-3">
              <h3 className="text-sm font-semibold">Modifier les quotas</h3>
